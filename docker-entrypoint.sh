...
if [[ $DEST_BRANCH_FOUND == "null" ]]; then
    ...
    if [[ $LAST_NEW_COMMIT != "null" ]]; then
        ...
        myCurl --request PUT --header "PRIVATE-TOKEN: $GITLAB_TOKEN" --header "SUDO: $GITLAB_USER_ID" "$GITLAB_API_URL/projects/$PROJECT_ID/merge_requests/$PROMOTION_MR_IID/merge" -d "merge_commit_message=chore(promotion): promote macroservice from $SOURCE_BRANCH branch to $DEST_BRANCH branch\n[promote commit]" | jq .
    else
